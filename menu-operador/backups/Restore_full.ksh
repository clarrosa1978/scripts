...
# Autor..............: C.A.S                                                  #
# Usuario ...........: root                                                   #
# Objetivo...........: Restorear un backup Full                               #
# Nombre del programa: Restore_full.ksh                                       #
# Descripcion........: Baja los datos de un full backup con el comando restore#
...
JOBLOG=$OPERADOR_LOG/Restore_full.log
BLOCK_NEW=1024
MODIFICADO=0
RC=0
  ...
  read nada
    ...
    if [ $? -ne 0 ]
     then
      ...
      restore -qdvf /dev/$TAPE 1>$JOBLOG 2>/dev/null
      if [ $? -ne 0 ]
       then 
        echo "\t\t Error  -   Al realizar el restore"
        echo "\t\t < Pulse una tecla para continuar > \c"
        read nada
        RC=20  
       else 
        echo "\t\t Restore Finalizo OK"
        echo "\t\t < Pulse una tecla para continuar > \c"
        read RESPUESTA
      fi
