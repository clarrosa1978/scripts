...
# Nombre del programa: Backups_dir.ksh                                        #
...
JOBLOG=$OPERADOR_LOG/Backups_dir.log
BLOCK_NEW=1024
MODIFICADO=0
RC=0
     ...
     then
      ...
      if [ ! -d $DIRECTORIO ]
       then 
        ...
        cd $DIRECTORIO
        echo "\t\t Nombre del directorio a backupear"
        read DIR_BACKUP 
        tar -cvf /dev/$TAPE $DIR_BACKUP 1>$JOBLOG 2>/dev/null
        if [ $? -ne 0 ]
         then 
