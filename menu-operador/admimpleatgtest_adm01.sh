...
export HOST_DES="scdigitest01"

variable_xml()
{
export PATH_ORI_IMPLE="${PATH_ORI}/xml/testing"
...
#---------------------------------------------------------------------------------
#  Funcion para ver el archivo o directorio a enviar al scdigitest01
#---------------------------------------------------------------------------------
...
then
	...
	if [ "${DEPLOY}" = "Y" ]
	then
		ls -1 ${PATHDIR} | grep "COTODIGITAL_STORE_STAGING" >> ${ARCH_IMPLE}
		IMPLEMENTA="Directorio COTODIGITAL_STORE_STAGING"
	fi
fi
...
#---------------------------------------------------------------------------------
#   Funcion para seleccionar de una lista el archivo / Directorio a enviar al scdigitest01
#---------------------------------------------------------------------------------
...
#---------------------------------------------------------------------------------
#       Funcion para enviar archivos al scdigitest01
#---------------------------------------------------------------------------------
...
mv_deploy_scdigitest01()
{
set -x
# verifico usuario y grupo
if [ ${EXISTE} = 0 ]
then
       export ARCH_ORI="COTODIGITAL_STORE_STAGING"
	...
	if [ $? = 0 ]
   	    	then
                ...
                ssh ${HOST_DES} "cd ${PATH_DES_IMPLE} ; cp -pr ${ARCH_ORI} ${PATH_OLD} ; mv ${PATH_OLD}/${ARCH_ORI} ${PATH_OLD}/${ARCH_OLD} ; chown -R oraatg:oinstall ${PATH_OLD}/${ARCH_OLD}" #; mv ${PATH_OLD}/${ARCH_OLD} ${PATH_OLD}/${ARCH_ORI}"
        else
...
####################################################################################
         ...
         SCP_XML_IMPLE) titulo " Copiando el XML al scdigitest01.. Aguardar por favor"
                        variable_xml
                        crealista ${PATH_ORI_IMPLE}
                        selecciono ${NOMBRE_SELECCION}
                        scp_scdigitest01 ${NOMBRE_SELECCION}
                        ;;
      ...
      SCP_DEPLOY_IMPLE) titulo " Copiando el COTODIGITAL_STORE al scdigitest01.. Aguardar por favor"
                        variable_deploy
                        crealista ${PATH_ORI_IMPLE}
                        selecciono ${NOMBRE_SELECCION}
                        scp_deploy_scdigitest01 ${NOMBRE_SELECCION}
                        mv_deploy_scdigitest01
                        ;;
