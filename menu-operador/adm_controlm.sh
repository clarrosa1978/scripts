...
>${ARCH_TMP}
>${MENU_TMP}
su - ctmsrv -c "/home/ctm/server/ctm/exe_AIX/ctm_agstat -list \"*\"  | cut -d' ' -f2" >${ARCH_TMP}
...
# -------------------------------------------------------------------------------------------
	...
	UPAGINSTAL)     titulo "Levantado agentes creados en el servidor"
			...
			su - ctmsrv -c "/home/ctm/server/ctm/exe_AIX/ctm_agstat -UPDATE ${AGENTE} AVAILABLE" 
			teclacontinuar
			;;
	...
	DOWNAGINSTAL)	titulo "Bajando agentes creados en el servidor"
			listaragectm
			echo "Actualizando el agente ${AGENTE}"
			su - ctmsrv -c "/home/ctm/server/ctm/exe_AIX/ctm_agstat -UPDATE ${AGENTE} DISABLED" 
			teclacontinuar
			;;

        VERAGINSTAL)    titulo " Ver agentes creados en el servidor"
                        su - ctmsrv -c "/home/ctm/server/ctm/exe_AIX/ctm_agstat -list \"*\" "
			teclacontinuar
                        ;;
