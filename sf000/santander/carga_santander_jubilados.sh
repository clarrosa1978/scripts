...
# Objetivo...........: Carga lista blanca santander jubilados                 #
# Nombre del programa: carga_santander_jubilados.sh                           #
# Nombre del JOB.....: CARGASANTJUBILADOS                                     #
...
###############################################################################
export FECHA=${1}
export NOMBRE="carga_santander_jubilados"
...
then
	export ARCHIVO="`cd ${PATHAPL}/txt && ls -1 tarj_debito_jub_coto_*.txt`"
	...
	if [ $? != 0 ] && [ $? != 2 ]
   	then
		...
		grep -i ORA-01722 ${LSTSQLCTL}
                if [ $? != 0 ]
                then
                        Enviar_A_Log "WARNING - No se pudo cargar algunos registros en la base." ${LOGSCRIPT}
                        cat ${LSTSQLCTL}
		else
			Enviar_A_Log "AVISO - La carga del archivo de Santander termino OK." ${LOGSCRIPT}
                	Enviar_A_Log "FINALIZACION - OK." ${LOGSCRIPT}
