...
# Objetivo...........: Carga completo de Lista Negra de TCI                   #
...
export PATHBKP="${PATHAPL}/tci"
export PATHSQL="${PATHAPL}/sql"
export PATHLOG="${PATHAPL}/log"
export ARCHDATA="${PATHAPL}/listanegraresumentci.${FECHA}.txt"
...
if [ -s ${ARCHDATA}.gz ]
then
	Enviar_A_Log "AVISO - Existe el archivo de TCI: ${ARCHDATA}." ${LOGSCRIPT}
	gunzip ${ARCHDATA}.gz
	if [ $? != 0 ]
	then
	...
	if [ $? != 0 ]
   	then
		...
		exit 2
			...
			Enviar_A_Log "AVISO - La carga TCI termino OK." ${LOGSCRIPT}
