...
export FS_OS="/home:90 /tmp:90 /usr:90 /var:85 /opt:90 /usr/local:90 /proc:90 /expora:85 /ctm:85 /sfctrl:85 /sts:90 /tecnol:90 /transfer:90 /vtareserva:85 /avirus:90 /rman:80 /u02/oradata/CTRLM${NRO_SUC_3D}:98 /u02/oradata/SF${NRO_SUC_3D}:98 /dbfs_SF${NRO_SUC_3D}_archive:80 /depuraciones_mensuales:100"
export TOPE_OS=90
...
#set -x
	...
	then
		case $OS in
			AIX )  df -k $FS_CONTROL | grep -v Used |awk '{print $4 "\t$HOST_DESTINO"}' > $TEMPORAL;;
...
export ESTADO=""
	...
	export j=1
	export REG_TOTAL=$i
        echo "        `date`\t\t$DIR_SCRIPT/fs_control.sh"					>>$CUERPO
	echo "\t##########################################################################"	>>$CUERPO
        echo "\tFILESYSTEM\t\t\t | NODO    |\t    |    | PA | PP | STATUS"			>>$CUERPO
        echo "\t##########################################################################"	>>$CUERPO
