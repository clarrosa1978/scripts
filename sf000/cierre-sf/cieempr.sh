...
# Objetivo...........: Ejecutar el programa /sfctrl/sfgv/bin/cieempr          #
# Nombre del programa: cieempr.sh                                             #
# Nombre del JOB.....: CIEEMPR                                                #
# Descripcion........: Cierra la empresa de SF                                #
...
then
	${PATHPRG}/${PROGRAMA} ${MODEJE} >/dev/null
	if [ $? != 0 ]
	then 
		...
		if [ -s ${LOGPRG} ]
		then
	 		if [ `grep -c "cieempr: Terminaci\"n correcta del cierre de empresa" ${LOGPRG}` = 1 ]
			then
