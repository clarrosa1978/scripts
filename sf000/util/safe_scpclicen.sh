...
Check_Par 7 $@
[ $? != 0 ] && exit 1
if [ -f /tecnol/util/safe_rcp -a -x /tecnol/util/safe_rcp ]
then
         /tecnol/util/safe_rcp "$EXEC_FLAG" "$SOURCE_DIR" "$SOURCE_FILENAME" "$TARGET_HOST" "$TARGET_DIR" "$TARGET_FILENAME" "$ALTERN_DIR" "$ALTERN_FILENAME" "$RETRIES" "$RETRIES_DELAY" "$REM_USER" "$TARGET_OWNER" "$TARGET_GROUP" "$TARGET_MODE"
         ...
         then
		  ...
		  compress -f "$TARGET_DIR"/"$TARGET_FILENAME"
         fi
