...
drawVio ()
{
	set -A VioHostname $(grep $1 $configFile | grep 'lpar_env=vioserver' | cut -f 2 -d ',' | sort -u | cut -f 2 -d '=')
	...
	((vhostLine_x2=startx+sizeViox+100))	
        ...
        do
	        # draw VIO LPAR
       		set -A vhostAdap $(grep $1 $configFile | grep ${VioHostname[$a]} | grep 'vhost=' | cut -f 3 -d ',' | cut -f 2 -d '=')
		vhostAdap_count=${#vhostAdap[*]}   
       		set -A vfchosts $(grep $1 $configFile | grep ${VioHostname[$a]} | grep 'vfchost' | cut -f 4 -d ',')
		...
		lparId=$(grep $1 $configFile | grep ${VioHostname[$a]} | grep -v FC| grep 'adapter_type=server' | cut -f 3 -d ',' | cut -f 2 -d '=' | sort -u)   
       		set -A vhostSlot $(grep $1 $configFile | grep ${VioHostname[$a]} | grep 'vhost=' | cut -f 4 -d ',' | cut -f 2 -d '=' | cut -f 3 -d '.' | cut -f 3 -d '-' | cut -c 2-4)   
       		set -A vfchostSlot $(grep $1 $configFile | grep ${VioHostname[$a]} | grep 'vfchost' | cut -f 5 -d ',' | cut -f 3 -d '-')   
		set -A fcsAdap $(grep $1 $configFile | grep ${VioHostname[$a]} | grep 'WWPN=' | cut -f 3 -d ',' | cut -f 2 -d '=')
        	set -A fcsAdapLoc $(grep $1 $configFile | grep ${VioHostname[$a]} | grep 'WWPN=' | cut -f 4 -d ',' | cut -f 2 -d '=')
        	set -A fcsAdapWWPN $(grep $1 $configFile | grep ${VioHostname[$a]} | grep 'WWPN=' | cut -f 5 -d ',' | cut -f 2 -d '=' | cut -c 21-36)
        	fcsAdap_count=$(grep $1 $configFile | grep ${VioHostname[$a]} | grep -c 'WWPN=')
		set -A addPhys $(grep $1 $configFile | grep ${VioHostname[$a]} | grep 'io_adapter' | tr "=" "\n" | grep -v ${VioHostname[$a]} | grep -v Ethernet | grep -v 'PCI-to-PCI_bridge' | grep -v 'Fibre_Channel' | cut -f 1 -d ',')
		set -A addPhysType $(grep $1 $configFile | grep ${VioHostname[$a]} | grep 'io_adapter' | tr "=" "\n" | grep -v ${VioHostname[$a]} | grep -v Ethernet | grep -v 'PCI-to-PCI_bridge' | grep -v 'Fibre_Channel' | cut -f 2 -d ',' | cut -f 1 -d '_')
		...
		echo '<rect x="'$startx'" y="'$vio_y'" width="'$sizeViox'" height="'$sizeVioy'" fill="antiquewhite" style="stroke:black;stroke-width:1px" onclick="top.showMessage('$id')" onmouseover="evt.target.setAttribute('"'"'opacity'"'"', '"'"'0.5'"'"');" onmouseout="evt.target.setAttribute('"'"'opacity'"'"', '"'"'1'"'"');"/>' >> $pictureFile
...
#####################################################################
		...
		do
			...
			echo '<line x1="'$vhostLine_x1'" y1="'$vhostLine_y1'" x2="'$vhostLine_x2'" y2="'$vhostLine_y1'" style="stroke:black;stroke-width:1;" />' >> $pictureFile
...
#                        vhostLoc='U'$type'.'$model'.'$1'-V'$lparId'-C'${vhostSlot[$b]}'-T1'
#                        echo '<text x="'$text_x2'" y="'$text_y2'" style="font-size:10px">'$vhostLoc'</text>' >> $pictureFile
#                        remoteLpar=$(grep $1 $configFile | grep ${VioHostname[$a]} | grep 'slot_num='${vhostSlot[$b]} | grep 'adapter
#_type=server' | cut -f 9 -d ','| cut -f 2 -d '=')
                        remoteLparId=$(grep $1 $configFile | grep ${VioHostname[$a]} | grep 'slot_num='${vhostSlot[$b]} | grep 'adapter_type=server' | cut -f 10 -d ','| cut -f 2 -d '=')
			((hdisk_x=vhost_x1-80))
			id="'"${VioHostname[$a]}"_vhost"$b"'"
			echo '<ellipse id="'${VioHostname[$a]}'_vhost'$b'" cx="'$hdisk_x'" cy="'$vhostLine_y1'" rx="35" ry="12" fill="lightyellow" style="stroke:black;stroke-width:1px" onclick="top.showMessage('$id')" onmouseover="evt.target.setAttribute('"'"'opacity'"'"', '"'"'0.5'"'"');" onmouseout="evt.target.setAttribute('"'"'opacity'"'"', '"'"'1'"'"');"/> ' >> $pictureFile 
			((hdiskLine_x=hdisk_x+35))
			echo '<line x1="'$hdiskLine_x'" y1="'$vhostLine_y1'" x2="'$vhost_x1'" y2="'$vhostLine_y1'" style="stroke:red;stroke-width:1;" />' >> $pictureFile
			((text_x=hdisk_x-10))
...
#########################################################################
		 b=0
			...
			echo '<line x1="'$vfchostLine_x1'" y1="'$vfchostLine_y1'" x2="'$vhost_x1'" y2="'$vfchostLine_y2'" style="stroke:black;stroke-width:1;" />' >> $pictureFile
...
#########################################################################
		...
		do
			echo '<rect x="'$addPhys_x1'" y="'$addPhys_y1'" width="35" height="12" fill="powderblue" style="stroke:black;stroke-width:1px"/>' >> $pictureFile
...
drawClient ()
{
	...
	do
		...
		set -A clientVhost $(grep ${MaganedSystem[$x]} $configFile | grep 'lpar_name='${clientHostname[$b]} | grep -v 'adapter_type=server'| grep -v FC | cut -f 4 -d ',' | sort -u | cut -f 2 -d '=')
		clientVhost_count=${#clientVhost[*]}
		
		set -A clientVfchost $(grep ${MaganedSystem[$x]} $configFile | grep 'lpar_name='${clientHostname[$b]} | grep -v 'adapter_type=server'| grep FC | cut -f 5 -d ',' | sort -u | cut -f 2 -d '=')
		clientVfchost_count=${#clientVfchost[*]}

		set -A addPhysClient $(grep $1 $configFile | grep ${clientHostname[$b]} | grep 'io_adapter' | tr "=" "\n" | grep -v ${clientHostname[$b]} | grep -v Ethernet | grep -v 'PCI-to-PCI_bridge' | grep -v 'No_results' | cut -f 1 -d ',')
                set -A addPhysClientType $(grep $1 $configFile | grep ${clientHostname[$b]} | grep 'io_adapter' | tr "=" "\n" | grep -v ${clientHostname[$b]} | grep -v Ethernet | grep -v 'PCI-to-PCI_bridge' | cut -f 2 -d ',' | cut -f 1 -d ',')
		...
		do 
			echo '<rect x="'$client_x2'" y="'$client_y2'" width="35" height="12" fill="greenyellow" style="stroke:black;stroke-width:1px"/>' >> $pictureFile
			...
			then
			 ...
			 echo '<line x1="'$client_x2'" y1="'$client_y3'" x2="'$client_x3'" y2="'$client_y3'" style="stroke:black;stroke-width:1;" />' >> $pictureFile
                      	echo '<circle cx="'$client_x3'" cy="'$client_y3'" r="2.5" />' >> $pictureFile
			 echo '<line x1="'$client_x3'" y1="'$client_y3'" x2="'$client_x3'" y2="'$client_y4'" style="stroke:black;stroke-width:1;" />' >> $pictureFile
		...
		c=0
                ...
                do
                        echo '<rect x="'$client_x2'" y="'$client_y2'" width="35" height="12" fill="greenyellow" style="stroke:black;s
troke-width:1px"/>' >> $pictureFile
			...
			((text_y1=client_y2+25))
			 echo '<text x="'$text_x1'" y="'$text_y1'" style="font-size:10px">'$clientWWPN'</text>' >> $pictureFile
                        ...
                        then
                         ...
                         echo '<line x1="'$client_x2'" y1="'$client_y3'" x2="'$client_x3'" y2="'$client_y3'" style="stroke:black;stro
ke-width:1;" />' >> $pictureFile
                        echo '<circle cx="'$client_x3'" cy="'$client_y3'" r="2.5" />' >> $pictureFile
                         echo '<line x1="'$client_x3'" y1="'$client_y3'" x2="'$client_x3'" y2="'$client_y4'" style="stroke:black;stro
ke-width:1;" />' >> $pictureFile
		...
		do
			echo '<rect x="'$client_x3'" y="'$client_y3'" width="35" height="12" fill="powderblue" style="stroke:black;stroke-width:1px"/>' >> $pictureFile
                        ((text_x1=client_x3+5))
...
drawMessage ()
{
	set -A VioHostname $(grep $1 $configFile | grep 'adapter_type=server' |grep -v FC | cut -f 2 -d ',' | sort -u | cut -f 2 -d '=')
        VioHostname_count=${#VioHostname[*]}
	if [ $VioHostname_count == 0 ]
	then
		set -A VioHostname $(grep $1 $configFile | grep 'adapter_type=server' |grep FC | cut -f 3 -d ',' | sort -u | cut -f 2 -d '=')
        	VioHostname_count=${#VioHostname[*]}
	fi
        a=0
        while [ $a -lt $VioHostname_count ]
		...
		do
			...
			set -A vtd $(grep $1 $configFile | grep ${VioHostname[$a]} | grep ${vhostAdap[$b]} | tr "," "\n" | grep VTD | cut -f 2 -d '=')
			vtd_count=$(grep $1 $configFile | grep ${VioHostname[$a]} | grep ${vhostAdap[$b]} | tr "," "\n" | grep -c VTD)
			set -A backing $(grep $1 $configFile | grep ${VioHostname[$a]} | grep ${vhostAdap[$b]} | tr "," "\n" | grep 'BackingDevice' | cut -f 2 -d '=')
			set -A size $(grep $1 $configFile | grep ${VioHostname[$a]} | grep ${vhostAdap[$b]} | tr "VTD" "\n" | grep -v ',vhost='| cut -f 3 -d ',' | cut -f 2 -d '=')
			...
			do 
				...
				pepe='<tr><td>'${vtd[$c]}'</td><td>'${backing[$c]}'</td><td>'$pvid'</td><td>'$lunId'</td><td>'${size[$c]}'</td></tr>'
				((c=c+1))
			done
			string=$string' '$pepe'</table>"'
		...
		set -A lsnports $(grep $1 $configFile | grep ${VioHostname[$a]} | grep lsnports) 	
		...
		set -A vfchostAdap $(grep $1 $configFile | grep ${VioHostname[$a]} | grep ',FC,' | grep 'vfchost' | cut -f 4 -d ',')
...
######################################################################
	...
	set -A VioHostname $(grep ${MaganedSystem[$x]} $configFile | grep 'adapter_type=server' | cut -f 2 -d ',' | sort -u | cut -f 2 -d '=')
...
echo "<svg width=\""$sizePicturex"\" height=\""$pictureSize_y"\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\"
>" >> $sanPicture
...
#############################################################################
        ...
        echo '<rect x="'$legend_x2'" y="'$legend_y4'" width="10" height="10" fill="powderblue" style="stroke:black;stroke-width:1px"/>' >> $sanPicture
        ((legend_y5=legend_y2-10))
        echo '<rect x="'$legend_x2'" y="'$legend_y5'" width="10" height="10" fill="greenyellow" style="stroke:black;stroke-width:1px"/>' >> $sanPicture
        ...
        echo '<rect x="'$legend_x4'" y="'$legend_y4'" width="10" height="10" fill="antiquewhite" style="stroke:black;stroke-width:1px"/>' >> $sanPicture
        echo '<rect x="'$legend_x4'" y="'$legend_y5'" width="10" height="10" fill="lightyellow" style="stroke:black;stroke-width:1px"/>' >> $sanPicture

        ((legend_x7=legend_x4-10-150))
        ((legend_y7=legend_y4-10))
        echo '<rect x="'$legend_x7'" y="'$legend_y7'" width="470" height="70" fill="none" style="stroke:black;stroke-width:1px"/>' >> $sanPicture
        echo '<rect x="'$legend_x7'" y="'$legend_y7'" width="130" height="35" fill="none" style="stroke:black;stroke-width:1px"/>' >> $sanPicture
