...
do
	...
	typeset -Z3 NROSUC
 	while true
		...
		then
			EST=`sudo rsh ${HOST} '( ping -c1 '${IP}' 1>/dev/null && echo OK || echo ERROR )'`
                        if [ ${EST} = "ERROR" ]
                        then
			...
			ALIAS=`sudo rsh ${HOST} '( host PCBALANZAS || echo ERROR )'` 
			if [ "${ALIAS}" = "ERROR" ]
			then
