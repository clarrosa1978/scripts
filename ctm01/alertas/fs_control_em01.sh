...
export FS_OS="/usr:95 /var:90 /tmp:90 /var/adm/ras/platform:90 /home:90 /proc:90 /opt:90 /home/ecs:90 /home/oracle:95 /oradata/EM62:95 /home/ctmsrv:95 /home/ctmagt:95 /export:95 /tecno:95"

#################################################
#Lista de Filesystem exceptuados               #INCLUIR ACA LOS FS QUE SE DESEAN EXCEPTUAR 
#################################################
export FS_NO="/mkcd/cd_images"
...
Enviar_mail_alerta ()
{
	...
	if [ $SO = "Linux" ]	
		then
		cat $ARC_SAL | mutt -s "$MSG_SUBJECT" -b ${USER}@coto.com.ar
	else
		cat $ARC_SAL | mail -s "$MSG_SUBJECT"  ${USER}@coto.com.ar 
	fi
	done
}
...
export DIR_SCRIPT="/tecnica/alertas"
export ARCH_DAT="$DIR_SCRIPT/control.dat"
export SO=`uname`
export SRV=`hostname`
