...
# Interfaz por la cual se generaran las virtuales.
  ...
  # Esta es mi IP
  D_INTERNA='129.90.255.253'
  # Red Interna, mi red
  MISUBRED='129.90.0.0/255.255.0.0'
  MISUBRED1='129.90.255.0/255.255.255.0'
  MISUBRED2='128.2.0.0/16'
  
  # Mesa de Ayuda
CALLCENTER=172.16.47.0/24  
  ...
  ACORDOBA=172.16.53.252
  EPUENTES=128.2.107.157
  JCNEGRIS=172.16.53.199
  RMARTINEZ='128.2.108.32'
  FFONTANA='172.16.51.158'
 
  # Coto Digital.
  NTS090='129.90.250.250'
  ...
  NTSZE090='129.90.240.240'

  # FTP de camaras
  NTSALFRED3='128.2.101.72'
  REDFTP='129.90.22.0/24'
...
for servidor in $TARJETAS $ONLINE2 $PCOMAR $SF000
   do
...
echo '- Politica [ Habilito Oracle ]'
   ...
   for server in $DBAS $CALLCENTER $MAYUDA $NTSWEB8 $NTSINTRANET \
		 $PUCARA $GRIDDBA $NTSANDES $NTSSQL11 \
		 $GDM $AROBERTO $MMONTERO $OSIRIS $LAURA $RMARTINEZ \
		 ...
		 $NTSZE090 $NTS090 $ACORDOBA $EPUENTES $JCNEGRIS \
		 $FFONTANA 
    do
	for i in $LISTENER
   	 do
...
for server in $MISUBRED $MAYUDA $MISUBRED2 $MULTISTORE
    do
           for i in $HTTP
           do   
