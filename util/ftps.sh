...
then
	if [ ${PATHORI} != 'NULL' ]
	then
		/usr/bin/curl --disable-eprt -n -k --ftp-ssl ftp://${SERVER}/${PATHORI}/${ARCHORI} -o ${PATHDES}/${ARCHDES}
	else
		/usr/bin/curl --disable-eprt -n -k --ftp-ssl ftp://${SERVER}/${ARCHORI} -o ${PATHDES}/${ARCHDES}
	fi
	EXIT_STAT="$?"
        if [ "$EXIT_STAT" -ne 0 ]
        then
	...
	if [ ${OPER} = "P" ]
	then
		/usr/bin/curl --disable-eprt -T ${PATHORI}/${ARCHORI} -n -k --ftp-ssl ftp://${SERVER}/${PATHDES}/${ARCHDES}
		EXIT_STAT="$?"
		if [ "$EXIT_STAT" -ne 0 ]
        	then
