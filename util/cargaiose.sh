...
# Objetivo...........: Carga  completa de Lista Negra de IOSE                 #
...
export ARCHDATA="${PATHAPL}/iose.txt"
...
then
	...
	sqlldr ${USUARIO} control=${SQLCTL} log=${LSTSQLCTL} data=${ARCHDATA} rows=10000  direct=true
	if [ $? != 0 ]
   	then
		...
		exit 5
			...
			Enviar_A_Log "AVISO - La carga de ${ARCHDATA} termino OK." ${LOGSCRIPT}
			Enviar_A_Log "FINALIZACION - La carga de lista iose termino OK." ${LOGSCRIPT}
