...
export NOMBRE="bkp_fsys_rollos_archive"
...
if [ -x /usr/bin/dsmc ]; then
	sudo /usr/bin/dsmc archive "/${FS}/*" -subdir=yes -description="Backup Rollos Fecha "${FECHA}"" -deletefiles > ${LOGSCRIPT}
