...
# Nombre del programa: scp_experta.sh                                         #
# Nombre del JOB.....:                                                        #
# Descripcion........:                                                        #
# Modificacion.......: 18/01/2024                                             #
###############################################################################
...
[ $? != 0 ] && exit 1
if [ -f /tecnol/experta/scp_experta -a -x /tecnol/experta/scp_experta ]
then
	/tecnol/experta/scp_experta "$EXEC_FLAG" "$SOURCE_DIR" "$SOURCE_FILENAME" "$TARGET_HOST" "$TARGET_DIR" "$TARGET_FILENAME" "$ALTERN_DIR" "$ALTERN_FILENAME" "$RETRIES" "$RETRIES_DELAY" "$REM_USER" "$TARGET_OWNER" "$TARGET_GROUP" "$TARGET_MODE" "$USER"
