...
################################################################################
#
#      Script: safe_scp_tick.sh
#       Autor: Jorge Fernandez Garay
#    Ult.Mod.: 04/2007
# Descripcion: Ejecuta el safe_scp de JAF para transmitir Boletines de Tickets
#              siniestrados.
...
do
         ...
         then
                    TARGET_BOL_COPY=`ssh "$TARGET_HOST" ls "$TARGET_DIR/$bol_filename"|xargs -i basename {}`
         else
...
/tecnol/bin/safe_scp "$EXEC_FLAG" "$SOURCE_DIR" "$SOURCE_FILE_NAME" "$TARGET_HOST" "$TARGET_DIR" "$TARGET_FILE_NAME" "NULL" "NULL" "3" "300" "NULL" "$OWNER" "$GROUP" "$PERM"
EXIT_STAT="$?"
