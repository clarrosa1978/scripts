...
# Nombre del programa: safe_scp_dmz.sh                                        #
# Nombre del JOB.....:                                                        #
# Descripcion........:                                                        #
...
[ $? != 0 ] && exit 1
if [ -f /tecnol/bin/safe_scp_dmz -a -x /tecnol/bin/safe_scp_dmz ]
then
	/tecnol/bin/safe_scp_dmz "$EXEC_FLAG" "$SOURCE_DIR" "$SOURCE_FILENAME" "$TARGET_HOST" "$TARGET_DIR" "$TARGET_FILENAME" "$ALTERN_DIR" "$ALTERN_FILENAME" "$RETRIES" "$RETRIES_DELAY" "$REM_USER" "$TARGET_OWNER" "$TARGET_GROUP" "$TARGET_MODE" "$USER"
