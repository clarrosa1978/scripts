...
#  18.1			10/06/2021		  Antony,Godwin					Script Modified - Added "Unix" in Extract Application Version.
#  19.0			11/15/2021    	  Antony, Godwin				Script Modified - MOdifed code to remove user input while execution.
#  			    04/16/2022		  Dubey, Ritika					Script Modified- Updated code to extract /etc/krb5.conf and /ect/security/access.conf. 
#  20.0         05/19/2023        Dubey, Ritika                 Script Modified - Updated code to extract /etc/security/lastlog file.
#    		    05/25/2023        Dubey, Ritika                 Script Modified - Updating the extract script to capture the error log and datalog file in the format ACTT_AIX_errors.txt and ACTT_AIX_scriptflow.txt respectively.
#
#  Notice:
# ------------------------------------------------------------------------------------
...
echo "Extract Application Version|^|ACTT Unix_AIX" >> ACTT_CONFIG_SETTINGS.actt
...
clear
echo "Script Starting on AIX."
#read STRN
else
clear
echo "This Script is for AIX system and this OS is not AIX." >> $SCRIPTFLOW
echo "This Script is for AIX system and this OS is not AIX. Hence exiting the script"
echo -e "\n Kindly execute the script on AIX Operating System.\n"
exit 
#read STRN
fi
...
echo "Extraction completed. Output file is \"etc_sudoers_files.txt\"\n" | tee -a $SCRIPTFLOW
else
...
##### Extraction of /etc/security/access.conf #######
...
echo "/etc/ssh2/sshd2_config  file does not exists in the system" 2>> $ERRFILE
fi
